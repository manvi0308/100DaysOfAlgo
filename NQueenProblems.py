...
# A recursive function that recurs over the columns t get an optimal solution
def Nqueenssprolem(board, col): 
    ...
    for i in range(N): 
        ...
        if isSafe(board, i, col): #Checking for that prerequisite function
            ...
            if Nqueenssprolem(board, col + 1) == True: 
                return True
...
def placeQueens(): 
    ...
    if Nqueenssprolem(board, 0) == False: 
